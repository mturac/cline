...
`run_commands` accepts an optional top-level `timeout` in milliseconds. It applies to the entire tool call and is passed to both the SDK timeout wrapper and the command executor.
...
- default: `30000` ms
- maximum: `2147483647` ms
- type: positive integer
- scope: top-level only

Do not put `timeout` on individual command entries. Per-command timeout is not supported:
...
- `timeout_source`: `default_setting` or `tool_parameter`
...
- `timeout_origin`: `wrapper` or `executor`
- optional context when available: `session_id`, `agent_id`, `conversation_id`, `run_id`, `iteration`, `tool_call_id`
